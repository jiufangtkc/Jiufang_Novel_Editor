...
> 最後更新：2026-09-03（完成 Phase 26，當日目標與進度多設備持久化與日誌連動）。
...
| Phase 20 | 系統重構、模組化與狀態防護加固 | ✅ 已完成 | 拆分 `database_migrations.py` 與 `autosave_controller.py`，加固 UI 狀態連動防護，127/127 測試全過 |
| Phase 21 | 資料集卡片 Markdown 富文本渲染與格式化工具 | ✅ 已完成 | 右側卡片編輯器支援 Markdown 即時高亮、快捷工具列 (B/I/H/•/~S~) 與預覽切換，129/129 測試全過 |
| Phase 22 | 小說 Markdown 底層轉換中介與出版匯出升級 | ✅ 已完成 | 建立 `utils/markdown_converter.py`，Docx/ePub 出版排版轉換、正文減噪高亮，134/134 測試全過 |
| Phase 24 | 真・所見即所得 WYSIWYG 富文本編輯 | ✅ 已完成 | 主編輯器與卡片編輯器徹底隱藏 Markdown/LaTeX 標記符號，原生樣式切換與無損 Markdown 雙向同步，135/135 測試全過 |
| Phase 25 | Markdown 所見即所得段落邊距消除與 ePub 匯出規範化 | ✅ 已完成 | 消除 Qt 富文本預設 12px 邊距使小說排版緊湊自然，規範化 ePub 匯出段落轉換，136/136 測試全數通過 |
| Phase 26 | 當日目標與進度多設備持久化與日誌連動 | ✅ 已完成 | 專案存檔支援 daily_target_word_count, writing_logs 連動同步，143/143 測試全過 |
